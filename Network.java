...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        if (user != null) {
            for (int i = 0; i < userCount; i++) {
                // if its comparing the name to itself, move on to the next one
                if (users[i] == user) {
                    continue;
                }
                // if current mutuals is better than the max mutuals, update it as the best one
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++) {
            ...
            int currentFollowees = followeeCount(users[i].getName());

            if ( currentFollowees > mostFollowees) {
                mostPopular = users[i];
